...
export default function EditWeeklyContentPage() {
  ...
  const fetchWeeklyContent = async () => {
    try {
      ...
      if (response.ok) {
        const data: WeeklyContent = await response.json();
        setFormData({
          title: data.title,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Failed to update weekly content: ${error.error}`);
      }
    } catch (error) {
      console.error('Error updating weekly content:', error);
      alert('Failed to update weekly content');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="p-8">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">You don't have permission to edit weekly content.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Edit Weekly Content</h1>
          <p className="text-muted-foreground">
            Update weekly news or learning material
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Update the details for this weekly content item
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end space-x-4">
              ...
              <Button type="submit" disabled={loading}>
                {loading ? 'Updating...' : 'Update Weekly Content'}
              </Button>
            </div>
          </form>
        </CardContent>
