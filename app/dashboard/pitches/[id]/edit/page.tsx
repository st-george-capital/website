...
export default function EditInvestmentPitchPage() {
  ...
  const fetchPitch = async () => {
    try {
      ...
      if (response.ok) {
        const data: InvestmentPitch = await response.json();
        setFormData({
          title: data.title,
          company: data.company || '',
          sector: data.sector,
          subcategory: data.subcategory || '',
          pitchDate: new Date(data.pitchDate).toISOString().split('T')[0],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Failed to update investment pitch: ${error.error}`);
      }
    } catch (error) {
      console.error('Error updating investment pitch:', error);
      alert('Failed to update investment pitch');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="p-8">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">You don't have permission to edit investment pitches.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center space-x-4">
        <Link href="/dashboard/pitches" className="p-2 text-gray-700 hover:text-gray-900 transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-3xl font-bold">Edit Investment Pitch</h1>
          <p className="text-muted-foreground">
            Update investment pitch document
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Update the details for this investment pitch
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end space-x-4">
              ...
              <Button type="submit" disabled={loading}>
                {loading ? 'Updating...' : 'Update Investment Pitch'}
              </Button>
            </div>
          </form>
        </CardContent>
