...
- [ ] **DATA-02**: System ingests FRED macro series using ALFRED vintage API, ensuring each historical data point reflects only what was published at that date (no retroactive revisions)
...
- [ ] **DATA-04**: System ingests Alpha Vantage premium data (price history, earnings, economic indicators) using existing rate-limit-aware sequential fetch pattern
- [ ] **DATA-05**: System ingests Financial Modeling Prep (FMP) earnings revision history (10+ years of analyst estimate changes) for all universe equities and ETFs
- [ ] **DATA-06**: System ingests OECD leading indicator series (free API, no key) for country-level macro context
