  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white p-6 rounded-lg border">
        <h3 className="font-bold text-xl mb-4 text-gray-900">Cost of Capital (WACC)</h3>
        <p className="text-gray-700 mb-4">
          The model applies a WACC of <strong>{(outputs.wacc * 100).toFixed(2)}%</strong> using the Capital Asset Pricing Model (CAPM) 
          for cost of equity and the company's marginal cost of debt.
        </p>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div>
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr>
                  <th className="border border-gray-300 px-3 py-2 bg-blue-50 text-left font-semibold">Component</th>
                  <th className="border border-gray-300 px-3 py-2 bg-blue-50 text-right font-semibold">Value</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td className="border border-gray-300 px-3 py-2">Risk-Free Rate</td>
                  <td className="border border-gray-300 px-3 py-2 text-right">{(inputs.riskFreeRate * 100).toFixed(2)}%</td>
                </tr>
                <tr>
                  <td className="border border-gray-300 px-3 py-2">Equity Risk Premium</td>
                  <td className="border border-gray-300 px-3 py-2 text-right">{(inputs.equityRiskPremium * 100).toFixed(2)}%</td>
                </tr>
                <tr>
                  <td className="border border-gray-300 px-3 py-2">Beta</td>
                  <td className="border border-gray-300 px-3 py-2 text-right">{inputs.beta.toFixed(2)}</td>
                </tr>
                <tr className="bg-blue-50 font-semibold">
                  <td className="border border-gray-300 px-3 py-2">Cost of Equity</td>
                  <td className="border border-gray-300 px-3 py-2 text-right">{(outputs.costOfEquity * 100).toFixed(2)}%</td>
                </tr>
                <tr>
                  <td className="border border-gray-300 px-3 py-2">After-Tax Cost of Debt</td>
                  <td className="border border-gray-300 px-3 py-2 text-right">{(outputs.afterTaxCostOfDebt * 100).toFixed(2)}%</td>
                </tr>
                <tr>
                  <td className="border border-gray-300 px-3 py-2">Target Debt Ratio</td>
                  <td className="border border-gray-300 px-3 py-2 text-right">{(inputs.targetDebtRatio * 100).toFixed(1)}%</td>
                </tr>
                <tr className="bg-blue-100 font-bold">
                  <td className="border border-gray-300 px-3 py-2">WACC</td>
                  <td className="border border-gray-300 px-3 py-2 text-right">{(outputs.wacc * 100).toFixed(2)}%</td>
                </tr>
              </tbody>
            </table>
          </div>
          
          <div className="flex flex-col justify-center space-y-3 text-sm text-gray-700">
            <div>
              <strong>Cost of Equity Calculation:</strong><br />
              {(inputs.riskFreeRate * 100).toFixed(2)}% + ({inputs.beta.toFixed(2)} × {(inputs.equityRiskPremium * 100).toFixed(2)}%) 
              = {(outputs.costOfEquity * 100).toFixed(2)}%
            </div>
            <div>
              <strong>WACC Calculation:</strong><br />
              ({((1 - inputs.targetDebtRatio) * 100).toFixed(1)}% × {(outputs.costOfEquity * 100).toFixed(2)}%) + 
              ({(inputs.targetDebtRatio * 100).toFixed(1)}% × {(outputs.afterTaxCostOfDebt * 100).toFixed(2)}%) 
              = <strong>{(outputs.wacc * 100).toFixed(2)}%</strong>
            </div>
          </div>
        </div>
      </div>
